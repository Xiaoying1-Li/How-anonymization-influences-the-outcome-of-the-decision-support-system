...
RESULT2 = []
args = sys.argv
default_input_file = "/insurance.csv"
default_output_file = "/multimondrian/output.csv"
default_quality_index = [0, 1,3,2, 4, 5]
default_k = 2
    ...
    def anonymise(self, partdata, attribute_index):
        ...
        for attr in attribute_index:
            ...
            if attr in [0,3]:
                ...
                minmum_val1 = min(map(float,attr_set))
                maxmum_val1 = max(map(float, attr_set))
                summary.append("[%s-%s]" % (minmum_val1, maxmum_val1))
            elif attr in [1, 4, 5]:  # sex, smoker, region
                if len(attr_set) == 1:
                    summary.append(attr_set[0])  # all values are the same
                else:
                    summary.append("*")  # values are different
            else:  # for all other attributes, directly append the set
                summary.append(attr_set if len(attr_set) > 1 else attr_set[0])

        anon_data = []
        for data in partdata:
            anon_record = list(data)
            for j, q in enumerate(attribute_index):
                if q in [1, 4, 5] and summary[j] == "*":
                    anon_record[q] = "*"  # anonymize the value
                else:
                    anon_record[q] = summary[j]  # use the summarized value
            anon_data.append(anon_record)
